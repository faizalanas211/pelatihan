...
<div class="row g-4">
    ...
    <div class="col-12 mt-2">
        ...
        {{-- PAGINATION CUSTOM MANUAL --}}
        @if($pelatihan->total() > 0)
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mt-5 pt-2">
            <div class="small text-muted">
                <i class="bi bi-info-circle me-1"></i>
                Showing {{ $pelatihan->firstItem() }} to {{ $pelatihan->lastItem() }} of {{ $pelatihan->total() }} results
            </div>
            ...
            <div class="d-flex gap-1">
                ...
                @else
                    <a href="{{ $pelatihan->previousPageUrl() }}" class="px-3 py-1 rounded text-decoration-none" style="background: #f1f3f5; color: #495057; font-size: 0.8rem;">Previous</a>
                @endif

                {{-- Nomor Halaman --}}
                @php
                    $currentPage = $pelatihan->currentPage();
                    $lastPage = $pelatihan->lastPage();
                    $start = max(1, $currentPage - 1);
                    $end = min($lastPage, $currentPage + 1);
                    
                    if ($start > 1) {
                        echo '<a href="'.$pelatihan->url(1).'" class="px-2 py-1 rounded text-center text-decoration-none" style="min-width: 32px; background: #f1f3f5; color: #495057; font-size: 0.8rem;">1</a>';
                    ...
                    for ($i = $start; $i <= $end; $i++) {
                        if ($i == $currentPage) {
                            ...
                            echo '<a href="'.$pelatihan->url($i).'" class="px-2 py-1 rounded text-center text-decoration-none" style="min-width: 32px; background: #f1f3f5; color: #495057; font-size: 0.8rem;">'.$i.'</a>';
                        }
                    }
                    
                    if ($end < $lastPage) {
                        if ($end < $lastPage - 1) echo '<span class="px-1" style="color: #adb5bd;">...</span>';
                        echo '<a href="'.$pelatihan->url($lastPage).'" class="px-2 py-1 rounded text-center text-decoration-none" style="min-width: 32px; background: #f1f3f5; color: #495057; font-size: 0.8rem;">'.$lastPage.'</a>';
                    }
                @endphp

                {{-- Next --}}
                @if($pelatihan->hasMorePages())
                    <a href="{{ $pelatihan->nextPageUrl() }}" class="px-3 py-1 rounded text-decoration-none" style="background: #f1f3f5; color: #495057; font-size: 0.8rem;">Next</a>
                @else
